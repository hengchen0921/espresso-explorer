...
/**
 * Breville Bambino — the Bambino Plus with the automatic milk system removed
 * and $200 off. Same shell, same ThermoJet, same 54 mm brew path; the two
 * milk dials on the Plus become one manual steam selector, which is the only
 * visible difference on the front panel. 19.5 × 32 × 31 cm.
 */
const W = 0.195
const D = 0.32
const H = 0.31
...
export function BrevilleBambinoModel() {
  return (
    <group>
      <Shell
        width={W}
      ...
      {/* 1.4 L rather than the Plus's 1.9 — the same footprint, a shallower box. */}
      <WaterTank
      ...
      <Part id="control-panel">
        ...
        {/* One manual selector where the Plus carries two milk dials. It is a
            valve you open, not a temperature you choose — the whole $200. */}
        <SteamKnob position={[0.052, 0.232, PANEL]} radius={0.015} axis="z" />
      </Part>
    </group>
  )
}
...
const anchors: PartAnchor[] = [
  ...
  },
  {
    ...
    normal: normal(180),
    camera: frame([0, 0.215, -0.08], 198, 16, 0.56),
  },
  {
    partId: 'drip-tray',
...
export const brevilleBambino: MachineModelDefinition = {
  machineId: 'breville-bambino',
  source: { kind: 'primitive', Component: BrevilleBambinoModel },
